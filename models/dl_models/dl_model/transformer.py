...
class TransformerLayer(nn.Module):
    ...
    Args:
        input_size: the hidden feature size.
    ...
    Examples:
        ...
        >>> input = torch.randn(3, 128, 64)  # [batch size, sequence len, input_size]
    ...
    def __init__(self, input_size, heads=1, dropout=0.5, num_layers=1):
        super(TransformerLayer, self).__init__()
        self.transformer = nn.ModuleList(
            [TransformerBlock(input_size, heads, dropout) for _ in range(num_layers)]
        )
    ...
    def forward(
        ...
        Returns:
            emb: a tensor of shape [batch size, sequence len, input_size],
                containing the output features for each time step.
            cls_emb: a tensor of shape [batch size, input_size], containing
...
class Transformer(nn.Module):
    def __init__(self, input_size, hidden_dim, heads=1, dropout=0.5, num_layers=1, **kwargs):
        super(Transformer, self).__init__()
        self.hidden_dim = hidden_dim
        self.transformer_layer = TransformerLayer(input_size, heads, dropout, num_layers)
        # self.proj = nn.Linear(input_size, hidden_dim)
        self.nn_output = nn.Linear(input_size, 1)
        self.nn_outcome = nn.Linear(input_size, 1)
    ...
    def forward(self, x, mask=None):
        ...
        #     _, cur_out = self.transformer_layer(cur_x, cur_mask) # use cls token
        #     cur_out = self.proj(cur_out)
        #     out[:, cur_time, :] = cur_out
        # return out
        _, out = self.transformer_layer(x, mask)  # use cls token

        output = self.nn_output(self.drop(out))
        # outcome = self.sigmoid(self.nn_outcome(self.drop(out)))
